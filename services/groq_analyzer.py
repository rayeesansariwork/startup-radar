"""
Groq AI Analyzer - Extract company data from search results
"""
...
class GroqAnalyzer:
    """Use Groq AI to extract company information from search results"""
    
    def __init__(self):
        self.groq = Groq(api_key=settings.groq_api_key)
        logger.info("✅ Groq Analyzer initialized")
...
"""
            ...
            logger.info(f"🤖 Sending prompt to Groq (prompt length: {len(prompt)} chars)")
            ...
            logger.info(f"📨 Received response from Groq ({len(result_text)} chars)")
            ...
            if not companies or len(companies) == 0:
                logger.warning("⚠️ Groq returned EMPTY array!")
